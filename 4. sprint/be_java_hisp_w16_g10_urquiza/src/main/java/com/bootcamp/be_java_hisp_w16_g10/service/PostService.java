...
public class PostService implements IPostService {
    ...
    // Modif individual: Ingresa datos de promocion
    // Recibe un PromoPostReqDTO, aplica validaciones y finalemente guarda el post.
    @Override
    public void save(PromoPostReqDTO promoPostReqDTO) {
        this.userService.findById(promoPostReqDTO.getUserId());
        if(promoPostReqDTO.getDate().isAfter(LocalDate.now()))
            throw new ConstraintViolationException("The date of the post is in the future.");
        this.postRepository.save(Mapper.parseToPost(promoPostReqDTO));
    }

    // Modif. individual: Contar promociones que tiene disponible un determinado usuario
    @Override
    public PromoCountResDTO countPromos(Integer userId) {
        ...
        List<Post> promoPosts = this.postRepository.findByUserId(user.getId()).stream()
                .filter(Post::getHasPromo).collect(Collectors.toList());

        return Mapper.parseToPromoCountResDTO(user, promoPosts);
    }

    // Modif. individual: Mostrar posts de una cierta categoria hechos por un usuario.
    // Se asume que no pueden entregarse categorias inexistentes
    public PostListResDTO listCategoryPosts(Integer userId, Integer category) {
        User user = this.userService.findById(userId);
        if (user == null)
            throw new NotFoundException(String.format("The user with id: %s doesn't exist.", userId));

        List<Post> posts = this.postRepository.findByUserId(user.getId()).stream()
                .filter(post -> post.getCategory().equals(category)).collect(Collectors.toList());

        return Mapper.parseToPostListResDTO(user, posts);
    }
