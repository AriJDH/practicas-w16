...
import com.bootcamp.be_java_hisp_w16_g08_bracco.exception.*;
...
import com.bootcamp.be_java_hisp_w16_g08_bracco.util.MapperProduct;
...
public class UserService implements IUserService {
    ...
    @Override
    public ResponseUserCountOfProductsWithPromo getUserCountOfProductsWithPromo(int userId) {
        User user= getUserIfExist(userId);
        List<Post> posts = user.getPostMade().stream().filter(post -> post.isHasPromo()).collect(Collectors.toList());
        ResponseUserCountOfProductsWithPromo response = new ResponseUserCountOfProductsWithPromo(user.getUserId(), user.getName(), posts.size());
        return response;
    }

    @Override
    public ResponsePromoDto getUserPromoList(int userId) {
        User user = getUserIfExist(userId);
        List<Post> promoPosts = user.getPostMade().stream().filter(post -> post.isHasPromo()).collect(Collectors.toList());


        List<PromoDto> promoPostDto = promoPosts.stream().map(post ->
                        new PromoDto(post.getPostId(),post.getDate(),
                                new ProductDto(post.getProduct().getId(),post.getProduct().getName(),
                                        post.getProduct().getType(), post.getProduct().getBrand(),
                                        post.getProduct().getColor(), post.getProduct().getNotes()), post.getCategory(),
                                        post.getPrice(), post.isHasPromo(),post.getDiscount())).collect(Collectors.toList());

        ResponsePromoDto  response = new ResponsePromoDto(user.getUserId(),user.getName(),promoPostDto);
        return response;
    }
