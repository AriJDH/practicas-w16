...
public class SocialMeliController {
    ...
    public ResponseEntity<Void> addPostWithPromo(@RequestBody PostDtoWithPromo post){
        postService.addPostWithPromo(post);
        return new ResponseEntity<>(HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/count")
    public ResponseEntity<ProductWithPromoCountDto> getProductsWithPromo(@RequestParam int userId){
        return new ResponseEntity<>(userService.getProductsWithPromo(userId), HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/list")
    public ResponseEntity<ProductsListWithPromoDto> productsListWithPromoDto(@RequestParam int userId){
        return new ResponseEntity<>(userService.productsListWithPromoDto(userId), HttpStatus.OK);
    }
}
