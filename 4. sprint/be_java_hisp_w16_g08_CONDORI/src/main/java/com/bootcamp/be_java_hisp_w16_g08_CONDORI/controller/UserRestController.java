...
import com.bootcamp.be_java_hisp_w16_g08_CONDORI.service.IUserService;
...
public class UserRestController {
    ...
    public ResponseEntity<PostPromotionCountDTO> publishPromoPost(@RequestParam int user_id){
        return new ResponseEntity<>(userService.countPostPromotion(user_id), HttpStatus.OK);
    }

    @GetMapping("/products/promo-post/list")
    public ResponseEntity<PostPromotionDTO> getPostPromotion(@RequestParam int user_id) {
        return new ResponseEntity<>(userService.getPostPromo(user_id), HttpStatus.OK);
    }

    @GetMapping("/products/all-post")
    public ResponseEntity<AllPostDTO> getAllPost(@RequestParam int user_id) {
        return new ResponseEntity<>(userService.getAllPost(user_id), HttpStatus.OK);
    }
}
