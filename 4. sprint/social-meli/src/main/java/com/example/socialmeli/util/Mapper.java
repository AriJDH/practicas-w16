...
public class Mapper {
    ...
    public static PostUS10 mapperToPromoPost(PostUS10DTO postUS10DTO){
        PostUS10 newPost = new PostUS10();
        newPost.setUserId(postUS10DTO.getUserId());
        newPost.setProduct(mapperToProduct(postUS10DTO.getProduct()));
        newPost.setDate(postUS10DTO.getDate());
        newPost.setCategory(postUS10DTO.getCategory());
        newPost.setPrice(postUS10DTO.getPrice());
        newPost.setHasPromo(postUS10DTO.isHasPromo());
        newPost.setDiscount(postUS10DTO.getDiscount());
    ...
    public static CountProductsDTO mapperToCountProductsDTO(User user, int cantidadProductos){
        CountProductsDTO countProductsDTO = new CountProductsDTO();
        countProductsDTO.setUserId(user.getUserId());
        countProductsDTO.setUserName(user.getUserName());
        countProductsDTO.setPromoProductsCount(cantidadProductos);

        return countProductsDTO;
    }

    public static ListPostsDTO mapperToListProductsDTO(User user, List<PostUS10> listaPosts){
        ListPostsDTO listPostsDTO = new ListPostsDTO();
        listPostsDTO.setUserId(user.getUserId());
        listPostsDTO.setUserName(user.getUserName());
        listPostsDTO.setPosts(listaPosts);
