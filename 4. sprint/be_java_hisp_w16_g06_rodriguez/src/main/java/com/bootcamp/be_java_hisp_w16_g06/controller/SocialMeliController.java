...
public class SocialMeliController {
    ...
    @Autowired
    IServiceIndividual serviceIndividual;
    ...
    public ResponseEntity<ResponseDTO> US0010(@RequestBody RequestPostPromotionDTO requestDto) {
        serviceIndividual.registerPostPromotion(requestDto);
        return new ResponseEntity<>(new ResponseDTO("Post created successfull", 200), HttpStatus.valueOf(200));
    }

    //US 0011: Obtener la cantidad de productos en promoción de un determinado vendedor
    @GetMapping("/products/promo-post/count")
    public ResponseEntity<CountPromotionDTO> US0011(@RequestParam int user_id) {
        return new ResponseEntity<>(serviceIndividual.getCountPromotionDTO(user_id), HttpStatus.OK);
    }
