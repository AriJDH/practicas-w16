...
import com.example.socialmeli.exception.PostWithoutPromotionException;
...
public class PostService implements IPostService {
    ...
    public void addPostWithPromotion(PostWithPromotionDto postWithPromotionDto){
        User user = userRepository.getById(postWithPromotionDto.getUserId());
        if (user == null) {
            throw new UserNotFoundException("El usuario con codigo " + postWithPromotionDto.getUserId() + " no existe.");
        }
        if(!postWithPromotionDto.isHasPromo() || postWithPromotionDto.getDiscount() <= 0){
            throw new PostWithoutPromotionException("La publicación no tiene ninguna promoción.");
        }

        Post postWithPromotion = Mapper.mapperToPost(postWithPromotionDto);
        this.postRepository.add(postWithPromotion);
    }
    @PostConstruct
