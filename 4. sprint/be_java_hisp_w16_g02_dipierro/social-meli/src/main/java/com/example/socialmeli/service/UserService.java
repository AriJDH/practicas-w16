...
public class UserService implements IUserService {
    ...
    public ProductWithPromotionCountDto getProductWithPromotionCount(int userId){
        User user = getById(userId);
        if (user == null) {
            throw new UserNotFoundException("El usuario con codigo " + userId + " no existe.");
        }

        List<Post> posList = postRepository.getAll();
        long productWithPromotionCount = postRepository.getAll().stream()
                                                                    .filter(post -> post.getUserId() == user.getUserId() && post.getHasPromo())
                                                                    .count();

        return Mapper.mapperToProductWithPromotionCountDto(user, (int)productWithPromotionCount);
    }
    public ProductWithPromotionListDto getProductWithPromotionList(int userId){
        User user = getById(userId);
        if (user == null) {
            throw new UserNotFoundException("El usuario con codigo " + userId + " no existe.");
        }

        List<Post> userProductWithPromotionList = postRepository.getAll().stream()
                                                                            .filter(post -> post.getUserId() == user.getUserId() && post.getHasPromo())
                                                                            .collect(Collectors.toList());

        return Mapper.mapperToProductWithPromotionListDto(user, userProductWithPromotionList);
    }
    @PostConstruct
