...
public class CodigoMorseRestController {
    ...
    public String morseToEspanol(@PathVariable String codigoRecibido){
        ...
        StringBuilder palabraEspanol = new StringBuilder();//me genera una cadena de caracteres
        llenarMorse();
        System.out.println(morseToEspañol);
        System.out.println(morseToEspañol.toString());

        List<String []> letrasPorPalabra =new ArrayList<>(); //creo un array

        String [] palabras = codigoRecibido.split("   "); //separo la palabra

        for (int i = 0;i<palabras.length;i++){
            String [] letras = palabras[i].split(" "); //separo la letra
