...
public class PostService implements IPostService {
    ...
    public List<Post> loadPostsFromDatabase() {
        File archivo = null;
        try {
            archivo = ResourceUtils.getFile("classpath:posts.json");
        ...
        try {
            posts = objectMapper.readValue(archivo, typeReference);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return posts;
    ...
    public void initPostData() {
        if (postRepository.getAll().isEmpty()) {
            ...
            List<Post> newPosts = this.loadPostsFromDatabase();

            for (Post posteos: newPosts){
                postRepository.add(posteos);
            }
        }
    }
