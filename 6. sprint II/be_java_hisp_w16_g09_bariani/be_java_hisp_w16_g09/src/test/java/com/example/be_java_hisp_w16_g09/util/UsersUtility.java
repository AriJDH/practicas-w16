...
public class UsersUtility {
    ...
    public static UserFollowedDto getUserFollowedDtoForUserId1(){
        MapperUtil mapperUtil = new MapperUtil();
        User userA = new User(1, "Ricardo", new ArrayList<>(), new ArrayList<>());
        User userB = new User(2, "Nicolas", new ArrayList<>(), new ArrayList<>());

        userA.addFollowed(userB);
        userB.addFollower(userA);

        return new UserFollowedDto(userA.getUserId(), userA.getUserName(), mapperUtil.mapList(userA.getFollowing(), SimpleUserDto.class));
    }
}
