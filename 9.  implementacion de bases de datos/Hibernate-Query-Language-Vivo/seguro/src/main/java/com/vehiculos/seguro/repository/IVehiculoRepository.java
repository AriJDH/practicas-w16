...
public interface IVehiculoRepository extends JpaRepository<Vehiculo, Long> {
    ...
    @Query("select v from Vehiculo v where v.cantRuedas > 4 and v.anioFabricacion = year(current_date)")
    ...
    @Query("select v from Vehiculo v join v.siniestros s where s.perdidaEconomica > 10000 group by v")
    List<Vehiculo> findAllVehiculosWithPerdidaEconomicaMayorDiezmil();
